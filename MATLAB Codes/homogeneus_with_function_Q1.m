...
clear;
close all;
Pbh=input('input bottom hole pressure(psia):');
...
qg_prim=input('input gas rate (scf/day):');
...
for i=1:N
%i=1;
%    T1=117.72+0.0128*(dL *(i-1));
%    T2=117.72+0.0128*(dL*i);
%    Tavg=(T1+T2)/2+460;%R
Tavg=639.7;
   Pavg=Pbh;%psia
...
MUl=MUo*fo+MUw*fw;
ZG=ZG_o*fo+ZG_w*fw;
dpdztotal1=dp_homogeneus_function_Q1(Vsl,MUl,Vsg,MUg,ROl,d1,gc,ROg,g,a1);
Pt(i+1)=Pavg-dpdztotal1*dL;
Pbh=Pt(i+1);
end
H=(N:-1:0).*dL;
plot(Pt,H);
